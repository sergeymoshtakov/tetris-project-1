...
namespace ConsoleApp1
{
    class Game
    {
        ...
        public void CreateFigure() // создание фигуры
        {
            // фигуры
            int[,] shapeI = { { 1, 1, 1, 1 } };
            int[,] shapeO = { { 2, 2 }, { 2, 2 } };
        ...
        public void DrawBoard() // отрисовка экрана
        {
            // отрисовка
            Console.Clear();
        ...
        public void ErraseFull() // удаление строк
        {
            // проверка строк
        ...
        public void EndGame() // pавершаем игру
        {
        ...
        public void HandleAction() // отслеживаем нажатия на клавиатуру
        {
            if (Console.KeyAvailable)
            {
                ...
                {
                    currentShape.CurrentShape = Shape.Rotate(currentShape.CurrentShape);
                }
        ...
        public void InitializePlayer() // созданм игрока
        {
        ...
        public void AddPlayerToList() // добавляем игрока в банк
        {
            DataBank.WriteToFile(player);
        }

        public void ShowTopPlayers() // показываем лучших игроков
        {
            ...
            for(int i = 0; i < players.Count; i++)
            {
                if(i == 0)
                {
                    Console.WriteLine($"{players[i].Name} - {players[i].Score} - Победидеть!!!");
                }
                else
                {
        ...
        public void ShowRules() // показываем правила игры
        {
            Console.Clear();
            Console.WriteLine("Правила" +
                "\n1 - Для перемещения используем правую и левую стрелки" +
                "\n2 - Для поворота фигуры нажать стрелку вверх" +
                "\n3 - Для выхода нажать пробел" + 
                "\n4 - При заполнении рядка он пропадае" +
                "\n5 - За каждый рядок начисляется 10 баллов" +
                "\n6 - Победитель тот, кто набрал больше всего баллов" +
                "\nПриятной игры)");
            Console.ReadKey();
            Console.Clear();
        }
